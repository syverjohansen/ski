...
def get_table(link):
	...
	if(distance=="Duathlon"):
		technique = "P"
		table = [date, city, country, distance, 1, technique, season, race_num]
		return table
	if(distance=="Sprint"):
			#technique = body[3].text.split(" ")
			technique = race.split(" ")
			technique = technique[1][0]
			table = [date, city, country, distance, ms, technique, season, race_num]
	...
	if(int(year)>1985 and distance!="Rel"):
		try:
			...
			table = [date, city, country, distance, ms, "N/A", season, race_num]
			return table
	else:
		table = [date, city, country, distance, ms, "N/A", season, race_num]
...
def construct_df(tables, skiers, sex):
	...
	table_df = pd.DataFrame(tables, columns=["Date", "City", "Country", "Distance", "MS", "Technique", "Season", "Race"])
	table_df['Sex'] = sex
	...
	empty_df = pd.DataFrame(columns=['Date', 'City', 'Country', 'Distance', 'MS', 'Sex', 'Technique', 'Season', 'Race','Place', 'Skier', 'Nation', 'ID'])
...
def main():
	...
	writer = pd.ExcelWriter("/Users/syverjohansen/ski/elo/python/ski/excel365/multithread_demo_all.xlsx")
