...
def get_table(worldcup_page):
	...
	while(len(to_worldcup_page)>0):
		...
		try:
			...
			w0+=1
			pass
	#I can't imagine why it would be greater than 1, but I'll keep it here anyways
...
def get_worldcup():
	...
	for a in range(1924,2025):
		...
		for i in range(0, len(men_worldcup_soup0.find_all('a', {'title':'Results'}, href = True)), 2):
   			b = men_worldcup_soup0.find_all('a', {'title':'Results'}, href = True)[i]
   			men_worldcup_page1.append('https://firstskisport.com/cross-country/'+b['href'])
		for i in range(0, len(ladies_worldcup_soup0.find_all('a', {'title':'Results'}, href = True)), 2):
   			b = ladies_worldcup_soup0.find_all('a', {'title':'Results'}, href = True)[i]
...
worldcup = [item for sublist in worldcup for item in sublist]


data = []
for row in worldcup:
    date, city, country, category, sex, distance, ms, technique, places, skiers, nations, ski_ids = row
    for place, skier, nation, ski_id in zip(places, skiers, nations, ski_ids):
        data.append({'Date': date, 'City': city, 'Country': country, 'Category': category, 'Sex': sex,
                     'Distance': distance, 'MS': ms, 'Technique': technique, 'Place': place,
                     'Skier': skier, 'Nation': nation, 'ID': ski_id})
