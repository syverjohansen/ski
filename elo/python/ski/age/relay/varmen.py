...
mendf = pd.read_pickle("~/ski/elo/python/ski/age/relay/excel365/mendf.pkl")
...
def distance(mendf, distances):
   # print(pd.unique(mendf['distance']))
    if(distances=="Sprint"):
        ...
        mendf = mendf.loc[(mendf['distance']=="Sprint") | (mendf['distance']=="Ts")]
...
def discipline(mendf, discipline):
    if(discipline == "F"):
        ...
        mendf = mendf.loc[mendf['discipline']!="P"]
        mendf = mendf.loc[mendf['discipline']!="F"]
        mendf = mendf.loc[mendf['distance']!="Rel"]
       # mendf = mendf.loc[mendf['distance']!="Ts"]
    ...
    for season in range(len(seasons)):
        ...
        for idd in ski_ids_s:
            ...
            endf = pd.DataFrame([[endseasondate, "Summer", "Break", "end", "M", 0, 0, None, 0
                , endname, endnation, idd ,seasons[season], 0, 0,0,0,endpelo, endelo]], columns = menelodf.columns)
...
varmenelo.to_pickle("~/ski/elo/python/ski/age/relay/excel365/varmen_classic_k.pkl")
varmenelo.to_excel("~/ski/elo/python/ski/age/relay/excel365/varmen_classic_k.xlsx")
