...
def process_races_single_mixed_relay(races_file: str = None) -> None:
    """
    Process single mixed relay races from races.csv, creating team CSV files
    ...
    if races_file and os.path.exists(races_file):
        ...
        races_path = '~/ski/elo/python/biathlon/polars/excel365/races.csv'
        ...
        try:
            ...
            # Filter to only single mixed relay races from today
            races_df = today_races[today_races['RaceType'] == 'Single Mixed Relay']
            print(f"Filtered to {len(races_df)} single mixed relay races for today")
        except Exception as e:
            print(f"Error loading races from {races_path}: {e}")
            traceback.print_exc()
            return
    
    # Process single mixed relay races - no gender separation for mixed events
    print(races_df)
    if not races_df.empty:
        process_single_mixed_relay_races(races_df)
    else:
        print("No single mixed relay races to process")

def process_single_mixed_relay_races(races_df: pd.DataFrame) -> None:
    """Process single mixed relay races, creating team CSV"""
    print("\nProcessing single mixed relay races")
    
    # Get path for output file
    team_output_path = "~/ski/elo/python/biathlon/polars/relay/excel365/startlist_single_mixed_relay_races_teams.csv"
    
    # Create output directory if it doesn't exist
    os.makedirs(os.path.dirname(os.path.expanduser(team_output_path)), exist_ok=True)
    
    # Get the ELO scores for both genders
    men_elo_path = "~/ski/elo/python/biathlon/polars/excel365/men_chrono.csv"
    women_elo_path = "~/ski/elo/python/biathlon/polars/excel365/ladies_chrono.csv"
    
    men_elo_scores = get_latest_elo_scores(men_elo_path)
    women_elo_scores = get_latest_elo_scores(women_elo_path)
    
    # Process each single mixed relay race
    all_teams_data = []
    ...
    for idx, (_, race) in enumerate(races_df.iterrows()):
        ...
        print(f"Processing single mixed relay race {idx+1}: {race['City']} ({race['Date']})")
        ...
        if not teams or len(teams) == 0:
            ...
            teams_data = process_single_mixed_relay_teams(teams, race, men_elo_scores, women_elo_scores)
    ...
    if all_teams_data:
        ...
        print(f"Saved {len(team_df)} single mixed relay teams to {team_output_path}")
    else:
        print(f"No team data generated")


def process_single_mixed_relay_teams(teams: List[Dict], race: pd.Series, men_elo_scores: pd.DataFrame, women_elo_scores: pd.DataFrame) -> List[Dict]:
    """
    Process single mixed relay teams and calculate team ELO scores
    ...
    for team in teams:
        ...
        # In single mixed relay, we have just 2 athletes (one male, one female)
        # Process each member
        processed_members = team['members'][:2] if len(team['members']) >= 2 else team['members']
        ...
        for member in processed_members:
            ...
            # For single mixed relay, position 1 is typically male, position 2 is female
            leg_based_male = (leg_position == 1)
            
            # Determine gender based on best match score and leg position
            if men_elo_match and women_elo_match:
                # If scores are close (within 10%), use leg position as tiebreaker
                if abs(men_match_score - women_match_score) < 10:
                    is_male = leg_based_male
                    elo_match = men_elo_match if is_male else women_elo_match
                    print(f"Scores close, using leg position to determine gender for {member_name}")
                ...
                # Fall back to leg position
                is_male = leg_based_male
                elo_match = None
                print(f"No ELO match found, using leg position for {member_name}: {'male' if is_male else 'female'}")
            
            # Select appropriate ELO data source and quartiles
        ...
        # Ensure we have both positions filled (for single mixed relay, typically 2 positions)
        for i in range(1, 3):  # Single mixed relay has 2 positions
            if f'Member_{i}' not in team_info:
                is_male = i == 1  # Position 1 is male
                quartiles = men_quartiles if is_male else women_quartiles
...
def generate_fallback_data(men_elo_scores: pd.DataFrame, women_elo_scores: pd.DataFrame, race: pd.Series) -> List[Dict]:
    ...
    for team_name in common_nations:
        ...
        # Initialize empty member fields for 2 legs of single mixed relay
        for i in range(1, 3):
            is_male = i == 1  # Position 1 is male
...
if __name__ == "__main__":
    ...
    process_races_single_mixed_relay(races_file)
