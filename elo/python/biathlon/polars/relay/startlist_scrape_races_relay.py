...
def process_races_relay(races_file: str = None) -> None:
    """
    Process relay races from races.csv, creating team CSV files
    
    Args:
        races_file: Optional path to a CSV containing specific races to process
    """
    print("Processing relay races")
    ...
    if races_file and os.path.exists(races_file):
        ...
        try:
            ...
            # Filter to only standard relay races from today
            races_df = today_races[today_races['RaceType'] == 'Relay']
            print(f"Filtered to {len(races_df)} standard relay races for today")
...
def process_gender_relay_races(races_df: pd.DataFrame, gender: str) -> None:
    ...
    team_output_path = f"~/ski/elo/python/biathlon/polars/relay/excel365/startlist_relay_races_teams_{gender}.csv"
...
if __name__ == "__main__":
    ...
    # Process relay races
    process_races_relay(races_file)
