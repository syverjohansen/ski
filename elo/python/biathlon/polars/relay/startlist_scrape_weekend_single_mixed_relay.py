...
def process_weekend_single_mixed_relay_races(races_file: str = None) -> None:
    """
    Process single mixed relay races for weekend, creating team CSV files
    
    Args:
        races_file: Optional path to a CSV containing specific races to process
    """
    print("Processing weekend single mixed relay races")
    ...
    if races_file and os.path.exists(races_file):
        ...
        try:
            ...
            # Filter to only single mixed relay races
            races_df = races_df[races_df['RaceType'] == 'Single Mixed Relay']
            print(f"Filtered to {len(races_df)} single mixed relay races")
            ...
            print(f"Found {len(races_df)} single mixed relay races on {next_date}")
...
def process_single_mixed_relay_races(races_df: pd.DataFrame) -> None:
    ...
    team_output_path = "~/ski/elo/python/biathlon/polars/relay/excel365/startlist_single_mixed_relay_weekend_teams.csv"
...
if __name__ == "__main__":
    ...
    # Process weekend single mixed relay races
    process_weekend_single_mixed_relay_races(races_file)
