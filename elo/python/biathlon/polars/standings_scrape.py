#!/usr/bin/env python3
"""
Biathlon standings scraper for FirstSkiSport.
...
MEN_URL = "https://firstskisport.com/biathlon/ranking.php?y=2026&cup=1"
WOMEN_URL = "https://firstskisport.com/biathlon/ranking.php?y=2026&g=w&cup=1"
OUTPUT_DIR = os.path.expanduser("~/ski/elo/python/biathlon/polars/excel365")
...
def extract_points(points_text):
    """Extract numeric points value from text like '(1291 pts)'"""
    if not points_text:
        return None
    match = re.search(r'\((\d+)', points_text)
    return int(match.group(1)) if match else None

def extract_shooting_percentage(percentage_text):
    """Extract shooting percentage from text like '97.14%'"""
...
def scrape_standings(url, gender):
    ...
    for row in rows:
        try:
            ...
            # Get prone shooting percentage
            prone_cell = row.select_one('td:nth-child(6)')
            prone_percentage = extract_shooting_percentage(prone_cell.text.strip()) if prone_cell else None
            
            # Get standing shooting percentage
            standing_cell = row.select_one('td:nth-child(7)')
            standing_percentage = extract_shooting_percentage(standing_cell.text.strip()) if standing_cell else None
            ...
            standings_data.append({
                ...
                'Points': points,
                'Wins': wins,
                'Prone_Percentage': prone_percentage,
