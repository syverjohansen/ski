...
# Define paths
base_path = Path('~/ski/elo/python/nordic-combined/polars/relay/excel365').expanduser()
output_path = Path('~/ski/elo/python/nordic-combined/polars/relay/excel365').expanduser()
...
def process_mixed_team(men_df):
    """Process Mixed Team data using men's team events"""
    print("Processing Mixed Team data (using men's team events)...")
    
    # Filter for Team races from men's data
    team_df = men_df.filter(pl.col("RaceType") == "Team")
    print(f"Found {team_df.shape[0]} rows for Mixed Team (from men's teams)")
    
    if team_df.shape[0] == 0:
        print("No Team data found in men's data")
        return None
    ...
    team_df_processed = (team_df_imputed
        ...
        .agg([
            ...
            # Sum Elo/Pelo values
            *[pl.sum(col).alias(f"Team_{col}") for col in elo_cols if col in team_df.columns],
            *[pl.sum(col).alias(f"Team_{col}") for col in pelo_cols if col in team_df.columns],
...
def process_team_sprint(combined_df):
    ...
    team_sprint_df = combined_df.filter(pl.col("RaceType") == "Team Sprint")
    ...
    # Group by team (Nation, Season, Race, Place, Sex)
    team_df = (team_sprint_imputed
        .group_by(["Nation", "Season", "Race", "Place", "Date", "City", "Country", "Event", "Sex"])
        .agg([
            # Count team members
            pl.len().alias("TeamSize"),
            # Take the first value for general race info
    ...
    print(f"Created team sprint dataframe with {team_df.shape[0]} rows")
    return team_df
...
def process_team(df, sex):
    ...
    team_df = df.filter(pl.col("RaceType") == "Team")
...
def main():
    ...
    # Combine men and ladies data for team sprint events
    combined_df = create_combined_df(men_chrono, ladies_chrono)
    
    # Process Mixed Team events (using men's team data)
    mixed_team_df = process_mixed_team(men_chrono)
    save_dataframe(mixed_team_df, "mixed_team_chrono")
    
    # Process Team Sprint events
