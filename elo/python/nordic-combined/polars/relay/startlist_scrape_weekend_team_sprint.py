...
def process_weekend_team_sprint_races(races_file: str = None) -> None:
    """
    Process team sprint races for the next weekend, creating team CSV files
    
    Args:
        races_file: Optional path to a CSV containing specific races to process
    """
    print("Processing Nordic Combined weekend team sprint races")
...
def process_gender_team_sprint_races(races_df: pd.DataFrame, gender: str) -> None:
    ...
    print(f"\nProcessing {gender} weekend team sprint races")
    
    # Get path for output file
    team_output_path = f"~/ski/elo/python/nordic-combined/polars/relay/excel365/startlist_team_sprint_weekend_{gender}.csv"
    
    # Create output directory if it doesn't exist
    os.makedirs(os.path.dirname(os.path.expanduser(team_output_path)), exist_ok=True)
    
    # Get the ELO scores
    elo_path = f"~/ski/elo/python/nordic-combined/polars/excel365/{gender}_chrono.csv"
    ...
    for idx, (_, race) in enumerate(races_df.iterrows()):
        ...
        print(f"Processing weekend team sprint race {idx+1}: {race['City']} ({race['Date']}) with ID: {race_id}")
        ...
        if not teams or len(teams) == 0:
            ...
            teams_data = process_weekend_team_sprint_teams(teams, race, gender, elo_scores)
...
def process_weekend_team_sprint_teams(teams: List[Dict], race: pd.Series, gender: str, elo_scores: pd.DataFrame) -> List[Dict]:
    """
...
if __name__ == "__main__":
    ...
    # Process weekend team sprint races
