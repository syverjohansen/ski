...
def process_weekend_team_races(races_file: str = None) -> None:
    """
    Process team races for the next weekend, creating team CSV files
    
    Args:
        races_file: Optional path to a CSV containing specific races to process
    """
    print("Processing Nordic Combined weekend team races")
    ...
    if races_file and os.path.exists(races_file):
        ...
        try:
            ...
            # Filter to only team races (but not team sprint)
            ...
            print(f"Found {len(races_df)} team races on {next_date}")
...
def process_gender_team_races(races_df: pd.DataFrame, gender: str) -> None:
    ...
    print(f"\nProcessing {gender} weekend team races")
    
    # Get path for output file
    team_output_path = f"~/ski/elo/python/nordic-combined/polars/relay/excel365/startlist_team_weekend_{gender}.csv"
    ...
    for idx, (_, race) in enumerate(races_df.iterrows()):
        ...
        print(f"Processing weekend team race {idx+1}: {race['City']} ({race['Date']}) with ID: {race_id}")
...
# For Nordic Combined, team data is structured differently
        # Look for member data directly in the team dictionary
        team_members = []
        
        # Extract members from the team dictionary (Nordic Combined structure)
        # Check if team has a Members list (proper structure)
        if 'Members' in team and team['Members']:
            team_members = team['Members']
        else:
            # Look for member data as individual keys in the team dictionary
            member_indices = set()
        ...
        # Ensure all positions are filled (usually 4 members per team)
        max_members = 4  # Standard team size for Nordic Combined
...
def generate_fallback_data(gender: str, elo_scores: pd.DataFrame, race: pd.Series) -> List[Dict]:
    ...
    for team_name in common_nations:
        ...
        for i in range(1, 5):  # Team typically has 4 members
...
if __name__ == "__main__":
    ...
    # Process weekend team races
    process_weekend_team_races(races_file)
