#!/usr/bin/env python3
"""
Nordic Combined standings scraper for FirstSkiSport.
...
# Updated URLs for Nordic Combined World Cup rankings
MEN_URL = "https://firstskisport.com/nordic-combined/ranking.php?y=2026&cup=1"
WOMEN_URL = "https://firstskisport.com/nordic-combined/ranking.php?y=2026&g=w&cup=1"
...
def extract_points(points_text):
    """Extract numeric points value from text like '(1506 pts)'"""
    if not points_text:
        return None
...
def scrape_standings(url, gender):
    ...
    """
    start_time = time()
    logger.info(f"Starting to scrape {gender}'s Nordic Combined World Cup standings from {url}")
    ...
    for row in rows:
        try:
            ...
            # Get points (4th column in the HTML sample)
            ...
            # Get wins (5th column in the HTML sample)
...
def main():
    ...
    logger.info("Scraping men's Nordic Combined World Cup standings...")
    men_data = scrape_standings(MEN_URL, 'men')
    save_to_csv(men_data, MEN_OUTPUT)
    
    # Scrape women's standings
    logger.info("Scraping women's Nordic Combined World Cup standings...")
    women_data = scrape_standings(WOMEN_URL, 'women')
    save_to_csv(women_data, WOMEN_OUTPUT)
    
    logger.info(f"Completed Nordic Combined scraping in {time() - start_time:.2f} seconds")
