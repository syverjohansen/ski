...
def process_weekend_team_races(races_file: str = None) -> None:
    ...
    """
    print("Processing Ski Jumping weekend team races")
    ...
    if races_file and os.path.exists(races_file):
        ...
        races_path = '~/ski/elo/python/skijump/polars/excel365/weekends.csv'
        
        try:
            races_df = pd.read_csv(races_path)
            print(f"Loaded {len(races_df)} races from {races_path}")
            
            # Filter to only team races (but not mixed team)
...
def process_gender_team_races(races_df: pd.DataFrame, gender: str) -> None:
    ...
    team_output_path = f"~/ski/elo/python/skijump/polars/relay/excel365/startlist_team_weekend_{gender}.csv"
    
    # Create output directory if it doesn't exist
    os.makedirs(os.path.dirname(os.path.expanduser(team_output_path)), exist_ok=True)
    
    # Get the ELO scores
    elo_path = f"~/ski/elo/python/skijump/polars/relay/excel365/{gender}_chrono.csv"
