...
output_dir = os.path.expanduser("~/ski/elo/python/skijump/polars/relay/excel365")
os.makedirs(output_dir, exist_ok=True)

def sex(df, sex):
    """Load data for specified sex (all competitions)"""
    if(sex=="M"):
        df = pl.read_csv("~/ski/elo/python/skijump/polars/relay/excel365/all_men_scrape.csv")
    else:
        df = pl.read_csv("~/ski/elo/python/skijump/polars/relay/excel365/all_ladies_scrape.csv")
...
def race_type(df, race_types):
    """Filter by race type - includes both individual and team events for hill sizes"""
    if not race_types or race_types == "null":
        return df
        
    if isinstance(race_types, list):
        # For each race type in the list, include both individual and team variants
        all_race_types = []
        for rt in race_types:
            all_race_types.append(rt)
            all_race_types.append(f"Team {rt}")
        df = df.filter(pl.col('RaceType').is_in(all_race_types))
    else:
        # Single race type - include both individual and team variant
        team_variant = f"Team {race_types}"
        df = df.filter(
            (pl.col('RaceType') == race_types) | 
            (pl.col('RaceType') == team_variant)
        )
...
def team_filter(df, include_teams):
    ...
    # If include_teams is 1, include all races (no filtering needed)
...
def event_filter(df, apply_filter):
    ...
    # Changed to use case-insensitive regex pattern instead of ignore_case parameter
...
def process_skier(idd, season_df, id_dict, discount, base_elo, seasons, season, sex, elo_df_columns):
    ...
    # Handle ski jumping specific fields - ensure proper Float64 types
    try:
        endlength1 = float(endskier['Length1'][-1]) if 'Length1' in endskier.columns and endskier['Length1'][-1] is not None else 0.0
        endlength2 = float(endskier['Length2'][-1]) if 'Length2' in endskier.columns and endskier['Length2'][-1] is not None else 0.0
        endpoints = float(endskier['Points'][-1]) if 'Points' in endskier.columns and endskier['Points'][-1] is not None else 0.0
    except (ValueError, TypeError):
        endlength1 = 0.0
        endlength2 = 0.0
        endpoints = 0.0
    
    # Create DataFrame with all columns in the correct order and proper types
    endf = pl.DataFrame({
        "Date": [endseasondate],
        "City": ["Summer"],
        "Country": ["Break"],
...
def elo(df, base_elo=1300, K=1, discount=.85):
    """
    Calculate ELO ratings for all Ski Jumping athletes in the dataset
    
    Args:
        df: DataFrame with race results
        base_elo: Base ELO rating for new athletes
    ...
    for season in range(len(seasons)):
        ...
        for race in range(len(races)):
            ...
            # Get race type - determine K adjustment based on race type and team events
            race_type = race_df['RaceType'][0]
            is_team_event = race_df['TeamEvent'][0]
            
            # Apply different K factors for different race types
            if is_team_event == 1:
                # Team events have weight divided by 4
                K_adj = K / 4
            else:
                # Individual events use normal K
                K_adj = K
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        data = {
            ...
            "race_type": None,
            "team_filter": 0,  # Default to excluding team events
            "date1": None,
            "date2": None,
            "city": None,
            "country": None,
    ...
    # Save CSV format with all_ prefix for all-competitions data
    elo_df.write_csv(f"{base_path}/all_{file_string}.csv")
