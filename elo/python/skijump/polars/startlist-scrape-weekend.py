...
def process_weekend_races() -> None:
    ...
    individual_races = next_races[
        (~next_races['RaceType'].str.contains("Team", na=False)) |
        (next_races['Sex'].isin(['M', 'L']))  # Individual men's or ladies' races
    ]
    
    regular_team_races = next_races[
        (next_races['RaceType'].str.contains("Team", na=False)) & 
        (~next_races['RaceType'].str.contains("Mixed", na=False)) &
        (next_races['Sex'] != 'Mixed') &
        (next_races['Sex'].isin(['M', 'L']))  # Men's or Ladies' team events
    ]
    
    mixed_team_races = next_races[
        (next_races['RaceType'].str.contains("Mixed Team", na=False)) |
        (next_races['Sex'] == 'Mixed')  # Mixed team events
    ]
    ...
    if not regular_team_races.empty:
        ...
        temp_file = "/tmp/weekend_team_races_skijump.csv"
        ...
        try:
            ...
            script_path = os.path.expanduser("~/ski/elo/python/skijump/polars/relay/startlist_scrape_weekend_team.py")
            subprocess.run(
    ...
    if not mixed_team_races.empty:
        ...
        temp_file = "/tmp/weekend_mixed_team_races_skijump.csv"
        ...
        try:
            ...
            script_path = os.path.expanduser("~/ski/elo/python/skijump/polars/relay/startlist_scrape_weekend_mixed_team.py")
            subprocess.run(
                [sys.executable, script_path, temp_file],
                check=True,
                env=env
...
def process_gender_races(races_df: pd.DataFrame, gender: str, host_nation: str) -> None:
    ...
    if consolidated_df is not None:
        ...
        output_path = f"~/ski/elo/python/skijump/polars/excel365/startlist_weekend_{gender}.csv"
