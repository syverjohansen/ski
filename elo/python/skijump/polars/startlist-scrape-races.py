...
def process_races() -> None:
    ...
    try:
        races_df = pd.read_csv('~/ski/elo/python/skijump/polars/excel365/races.csv')
    ...
    regular_team_races = races_df[
        (races_df['RaceType'].str.contains("Team", na=False)) & 
        (~races_df['RaceType'].str.contains("Mixed", na=False)) &
        (races_df['Sex'] != 'Mixed') &
        (races_df['Sex'].isin(['M', 'L']))  # Men's or Ladies' team events
    ]
    
    mixed_team_races = races_df[
        (races_df['RaceType'].str.contains("Mixed Team", na=False)) |
        (races_df['Sex'] == 'Mixed')  # Mixed team events
    ]
    ...
    # Process regular team races if available
    if not regular_team_races.empty:
        print("\n=== Processing Regular Team Races ===")
        # Save team races to temporary file for team script to process
        temp_file = "/tmp/races_team_skijump.csv"
        regular_team_races.to_csv(temp_file, index=False)
        ...
        try:
            ...
            # Call the regular team race script
            script_path = os.path.expanduser("~/ski/elo/python/skijump/polars/relay/startlist_scrape_races_team.py")
            subprocess.run(
                [sys.executable, script_path, temp_file],
                check=True,
                env=env
            )
            print("Successfully processed regular team races")
        except subprocess.CalledProcessError as e:
            print(f"Error calling regular team race script: {e}")
    ...
    if not mixed_team_races.empty:
        ...
        temp_file = "/tmp/races_mixed_team_skijump.csv"
        ...
        try:
            ...
            script_path = os.path.expanduser("~/ski/elo/python/skijump/polars/relay/startlist_scrape_races_mixed_team.py")
            subprocess.run(
                [sys.executable, script_path, temp_file],
                check=True,
                env=env
    ...
    r_script_path = os.path.expanduser('~/blog/daehl-e/content/post/skijump/drafts/race-picks.R')
...
def create_season_startlist(elo_path: str, race_info: pd.Series, gender: str, 
    ...
    try:
        ...
        hill_size = race_info.get('HillSize', determine_hill_size_from_race_type(race_type))
        
        print(f"Creating season startlist for {race_type} in {city}, {country} (Hill: {hill_size})")
        
        # Get chronological data to find current season skiers
        chrono_path = f"~/ski/elo/python/skijump/polars/excel365/{gender}_chrono.csv"
        try:
        ...
        for _, skier_row in current_skiers_df.iterrows():
            ...
            # Add ELO columns for ski jumping
            for col in ['Elo', 'Small_Elo', 'Medium_Elo', 'Normal_Elo', 'Large_Elo', 'Flying_Elo']:
                if col in elo_data:
                    row_data[col] = elo_data.get(col)
            
            # Add race-specific ELO based on hill size
            row_data['Race_Elo'] = get_race_specific_elo(elo_data, hill_size)
...
def create_race_startlist(race_id: str, elo_path: str, race_info: pd.Series, gender: str, 
    ...
    try:
        ...
        # Determine hill size from race metadata or race type
        hill_size = classify_hill_size_from_metadata(race_meta)
        if hill_size == 'Normal':  # If still default, try from race_info
            hill_size = race_info.get('HillSize', determine_hill_size_from_race_type(race_type))
        
        print(f"Determined hill size: {hill_size}")
        ...
        for athlete in athletes:
            ...
            if elo_match:
                ...
                skier_id = elo_data.get('ID', athlete['ID'])
            else:
            ...
            try:
                elo_priority = get_elo_priority(hill_size)
            except Exception as e:
            ...
            row_data = {
                ...
                'Length1': athlete.get('Length1', ''),
                'Length2': athlete.get('Length2', ''),
...
def process_gender_races(races_df: pd.DataFrame, gender: str, host_nation: str) -> None:
    ...
    # Get the ELO path
    elo_path = f"~/ski/elo/python/skijump/polars/excel365/{gender}_chrono.csv"
    ...
    if consolidated_df is not None:
        ...
        output_path = f"~/ski/elo/python/skijump/polars/excel365/startlist_races_{gender}.csv"
