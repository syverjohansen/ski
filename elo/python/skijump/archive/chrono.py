...
def ladies():
	lady_all_k = pd.read_excel('~/ski/elo/python/skijump/age/excel365/varladies_all_k.xlsx', sheet_name="Sheet1", header=0)
...
''	lady_tech_k = lady_tech_k.rename(columns = {'pelo':'tech_pelo', 'elo':'tech_elo'})'''
	...
	distance_col = list(lady_all_k['hill'])
...
'''
	...
	lady_all_k['distance'] = distance_col

	unique_ids = pd.unique(lady_all_k['id'])
	newdf = pd.DataFrame()
	for a in range(len(unique_ids)):
		skierdf = lady_all_k.loc[lady_all_k['id']==unique_ids[a]]
...
def men():
	man_all_k = pd.read_excel('~/ski/elo/python/skijump/age/excel365/varmen_all_k.xlsx', sheet_name="Sheet1", header=0)
	...
	distance_col = list(man_all_k['hill'])
	...
	man_all_k['distance'] = distance_col

	unique_ids = pd.unique(man_all_k['id'])
	newdf = pd.DataFrame()
	for a in range(len(unique_ids)):
		skierdf = man_all_k.loc[man_all_k['id']==unique_ids[a]]
...
ladiesdf = ladies()
ladiesdf.to_pickle("/Users/syverjohansen/ski/elo/python/skijump/age/excel365/ladies_chrono.pkl")
ladiesdf.to_excel("/Users/syverjohansen/ski/elo/python/skijump/age/excel365/ladies_chrono.xlsx")

mendf = men()		
mendf.to_pickle("/Users/syverjohansen/ski/elo/python/skijump/age/excel365/men_chrono.pkl")
mendf.to_excel("/Users/syverjohansen/ski/elo/python/skijump/age/excel365/men_chrono.xlsx")
