...
BASE_URL = "https://firstskisport.com/alpine/ranking.php"
...
def parse_standings(html_content: str, season: int, gender: str) -> List[Dict]:
    ...
    for row in rows[1:]:
        try:
            ...
            # Create standardized record (without Leg for Alpine)
            standing = {
...
def merge_with_existing_data(standings_df: pd.DataFrame, gender: str) -> pd.DataFrame:
    ...
    try:
        # Read existing race data - Updated path for Alpine
        ...
        # Ensure all required columns are present (without Leg for Alpine)
        expected_columns = [
...
def save_standings(standings: List[Dict], gender: str):
    ...
    # Ensure output directory exists - Updated path for Alpine
    output_dir = Path(os.path.expanduser("~/ski/ranks/alpine/excel365"))
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Convert date to datetime
    df['Date'] = pd.to_datetime(df['Date'])
    
    # Ensure integer types (removed Leg for Alpine)
    integer_columns = ['MS', 'Season', 'Race', 'Place', 'ID']
...
async def process_gender(gender: str):
    ...
    try:
        # Updated start year for Alpine (1967 instead of 1982)
        standings = await fetch_all_standings(1967, 2026, gender)
