...
def get_table(worldcup_page):
	...
	if("Olympic" in date_country[1]):
		category="Olympics"
	elif("World Championship" in date_country[1]):
		category="WSC"
	else:
		category = "WC"
...
#		return -1
	...
	table = [date, city, country, category, hill]
	return table
...
def get_worldcup():
	...
	for a in range(2024, 2025):
		print(a)
		men_worldcup_page0 = "https://firstskisport.com/ski-jumping/calendar.php?y="+str(a)
		ladies_worldcup_page0 = "https://firstskisport.com/ski-jumping/calendar.php?y="+str(a)+"&g=w"
		...
		for b in men_worldcup_soup0.find_all('a', {'title':'Results'}, href = True):
			if(title_results_count%2==0):
				men_worldcup_page1.append('https://firstskisport.com/ski-jumping/'+b['href'])
			title_results_count+=1
		
		title_results_count = 0
		for b in ladies_worldcup_soup0.find_all('a', {'title':'Results'}, href=True):
			if(title_results_count%2==0):
				ladies_worldcup_page1.append('https://firstskisport.com/ski-jumping/'+b['href'])
			title_results_count+=1

		men_standings_page0 = "https://firstskisport.com/ski-jumping/ranking.php?y="+str(a)
		ladies_standings_page0 = "https://firstskisport.com/ski-jumping/ranking.php?y="+str(a)+"&hva=&g=w"
		if(a>=2012):
			men_standings = get_standings(men_standings_page0, a)
			
			menwcs = [str(a)+'0500', "WC", "Standings", "table", "M", None, men_standings[0], men_standings[1],men_standings[2], men_standings[3]]
			menwc_standings.append(menwcs)
			ladies_standings = get_standings(ladies_standings_page0,a )
			ladieswcs = [str(a)+'0500', "WC", "Standings", "table", "L", None, ladies_standings[0], ladies_standings[1],ladies_standings[2], ladies_standings[3]]
	...
	for a in range(len(men_worldcup_page1)):
		...
		skiers = get_skier(men_worldcup_page1[a], hill)
		places = skiers[0]
		ski = skiers[1]
		nation = skiers[2]
		ski_ids = skiers[3]
		menwc = [date, city, country, category, sex, hill , places, ski, nation, ski_ids]
	...
	for a in range(len(ladies_worldcup_page1)):
		...
		skiers = get_skier(ladies_worldcup_page1[a], hill)
		places = skiers[0]
		ski = skiers[1]
		nation = skiers[2]
		ski_ids = skiers[3]
		ladieswc = [date, city, country, category, sex, hill,  places, ski, nation, ski_ids]
		#if(distance!="Rel" and distance!="Ts"):
...
workbook = xlsxwriter.Workbook("/Users/syverjohansen/ski/ranks/skijump/excel365/update_scrape.xlsx")
