...
BASE_URL = "https://firstskisport.com/biathlon/ranking.php"
...
def parse_standings(html_content: str, season: int, gender: str) -> List[Dict]:
    ...
    # Find the standings table - biathlon uses 'sortTabell tablesorter'
    ...
    for row in rows:
        try:
            ...
            # Create standardized record for Biathlon
            standing = {
...
def merge_with_existing_data(standings_df: pd.DataFrame, gender: str) -> pd.DataFrame:
    ...
    try:
        # Read existing race data - Updated path for Biathlon with relay support
        ...
        # Ensure all required columns are present (Biathlon format)
        expected_columns = [
...
def save_standings(standings: List[Dict], gender: str):
    ...
    # Ensure output directory exists - Updated path for Biathlon
    output_dir = Path(os.path.expanduser("~/ski/ranks/biathlon/excel365"))
    ...
    # Ensure integer types for Biathlon - handle None values
...
async def process_gender(gender: str):
    ...
    try:
        # Updated start year for Biathlon (1978)
        standings = await fetch_all_standings(1978, 2026, gender)
